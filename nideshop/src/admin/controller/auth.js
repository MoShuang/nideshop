...
const bucket = new upyun.Bucket(upyunConfig.bucket, upyunConfig.user, upyunConfig.pwd);
...
module.exports = class extends Base {
  ...
  async getSignAction() {
    ...
    if (!method || !path) {
      return this.success({status: false, msg: '参数错误'});
    }
    const headSign = upyun.sign.getHeaderSign(bucket, method, path);
    return this.success({status: true, data: headSign});
  }
};
